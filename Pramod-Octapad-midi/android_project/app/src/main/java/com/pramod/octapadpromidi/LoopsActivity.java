...
public class LoopsActivity extends Activity {
    ...
    private boolean loopServiceBound = false;
    private final ServiceConnection loopServiceConnection = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, android.os.IBinder service) {
            LoopService.LocalBinder binder = (LoopService.LocalBinder) service;
            LoopsActivity.this.loopService = binder.getService();
            LoopsActivity.this.loopServiceBound = true;
            LoopsActivity.this.refreshLoopUI();
        }

        @Override
        public void onServiceDisconnected(ComponentName name) {
            LoopsActivity.this.loopServiceBound = false;
            LoopsActivity.this.loopService = null;
        }
    };
    ...
    protected void onStart() {
        ...
        bindService(intent, this.loopServiceConnection, Context.BIND_AUTO_CREATE);
    }

    @Override // android.app.Activity
    protected void onStop() {
        super.onStop();
        if (this.loopServiceBound) {
            unbindService(this.loopServiceConnection);
            this.loopServiceBound = false;
    ...
    public void showPresetLoopsDialog() {
        new AlertDialog.Builder(this).setTitle("SELECT PRESET LOOP KIT").setItems(this.presetLoopNames, new DialogInterface.OnClickListener() { // from class: com.pramod.octapadpromidi.LoopsActivity.24            @Override // android.content.DialogInterface.OnClickListener
            public void onClick(DialogInterface dialog, int which) {
                LoopsActivity.this.loadPresetLoop(which);
            }
        }).setNegativeButton("Cancel", (DialogInterface.OnClickListener) null).show();
    }
    ...
    private void toggleLoop(final int index) throws IllegalStateException {
        ...
        if (this.loopServiceBound && this.loopService != null) {
            boolean result = this.loopService.toggleLoop(index, this.loopUris[index], this.isOneShotMode, this.isMultiMode, this.masterVolume, this.currentSpeed, this.currentPitch);
            if (!result) {
                this.txtLoopStatus.setText("ERROR LOADING LOOP " + (index + 1));
                return;
            }
            boolean playing = this.loopService.isLoopPlaying(index);
            updateLoopPadState(index, playing);
            this.txtLoopStatus.setText(playing ? "PLAYING LOOP " + (index + 1) : "LOOP " + (index + 1) + " STOPPED");
            if (!this.isMultiMode) {
                for (int i = 0; i < 8; i++) {
                    if (i != index) {
                        updateLoopPadState(i, this.loopService.isLoopPlaying(i));
                    }
                }
            }
            return;
    ...
    private void showEditOptions(final int index) {
        ...
        new AlertDialog.Builder(this).setTitle("EDIT LOOP " + (index + 1)).setItems(options, new DialogInterface.OnClickListener() { // from class: com.pramod.octapadpromidi.LoopsActivity.17            @Override // android.content.DialogInterface.OnClickListener
    ...
    public void renameLoopDialog() {
        ...
        new AlertDialog.Builder(this).setTitle("Enter Loop Name").setView(edt).setPositiveButton("OK", new DialogInterface.OnClickListener() { // from class: com.pramod.octapadpromidi.LoopsActivity.18            @Override // android.content.DialogInterface.OnClickListener
    ...
    public void showSaveLoopNameDialog() {
        ...
        edt.setText(this.currentLoopName);
        new AlertDialog.Builder(this).setTitle("Save Loop Group As").setView(edt).setPositiveButton("NEXT", new DialogInterface.OnClickListener() { // from class: com.pramod.octapadpromidi.LoopsActivity.19            @Override // android.content.DialogInterface.OnClickListener
            public void onClick(DialogInterface dialog, int which) {
    ...
    private void loadLoopFromFolder(Uri folderUri) throws IOException {
        ...
        DocumentFile dataFile = loopFolder.findFile("loop_data.json");
        if (dataFile != null) {
            try (InputStream inputStream = getContentResolver().openInputStream(dataFile.getUri())) {
                if (inputStream == null) {
                    return;
                }
    ...
    private void updateLoopPadState(int index, boolean playing) {
        if (index < 0 || index >= this.loopPads.length) {
            return;
        }
        if (playing) {
            this.loopPads[index].setBackgroundResource(R.drawable.pad_blue_glow_selector);
    ...
    private void refreshLoopUI() {
        ...
        for (int i = 0; i < 8; i++) {
            boolean playing = this.loopService.isLoopPlaying(i);
            updateLoopPadState(i, playing);
        }
        if (this.txtLoopStatus != null) {
            this.txtLoopStatus.setText("LOOP SERVICE " + (hasAnyLoopPlaying() ? "RUNNING" : "STOPPED"));
        }
    }

    private boolean hasAnyLoopPlaying() {
        if (!this.loopServiceBound || this.loopService == null) {
            return false;
        }
        for (int i = 0; i < 8; i++) {
            if (this.loopService.isLoopPlaying(i)) {
                return true;
            }
        }
        return false;
    ...
    public void handleMidiNoteOn(byte note, byte velocity) {
        ...
        runOnUiThread(new Runnable() { // from class: com.pramod.octapadpromidi.LoopsActivity.22            @Override // java.lang.Runnable
            public void run() throws IllegalStateException {
                ...
                if (i >= 0 && i < 8) {
                    ...
                    handler.postDelayed(new Runnable() { // from class: com.pramod.octapadpromidi.LoopsActivity.22.1
