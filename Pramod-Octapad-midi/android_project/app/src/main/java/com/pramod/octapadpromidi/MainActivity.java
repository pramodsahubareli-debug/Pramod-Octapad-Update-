...
public class MainActivity extends Activity {
    ...
    public void handleMidiNoteOn(byte note, byte velocity) {
        ...
        runOnUiThread(new Runnable() { // from class: com.pramod.octapadpromidi.MainActivity.3
            ...
            public void run() {
                ...
                handler.postDelayed(new Runnable() { // from class: com.pramod.octapadpromidi.MainActivity.3.1
                            @Override // java.lang.Runnable
                    public void run() {
                        try {
    ...
    public void copyPadSound(int fromPad, int toPad) {
        ...
        if (uri != null) {
            ...
            try {
                ...
                Toast.makeText(this, "Error copying sound: " + e3.getMessage(), 0).show();
                this.samples[toPad] = null;
                saveKitToMemory(this.kitIndex);
                Toast.makeText(this, "Copied PAD " + (fromPad + 1) + " -> PAD " + (toPad + 1), 0).show();
            }
            ...
            if (dataFile22 != null) {
                try {
                    ...
                    while (i4 < 8) {
                        ...
                        try {
                            eqMArray2.put(this.padEqMid[i4]);
                        } catch (Exception ignored) {
                        }
                        try {
                            eqLArray2.put(this.padEqLow[i4]);
                        } catch (Exception ignored) {
                        }
                        try {
                            chokeArray2.put(this.padChokeGroup[i4]);
                        } catch (Exception ignored) {
                        }
                        i4++;
                    }
    ...
    private void showKitListDialog(Uri folderUri) {
        try {
            ...
            new AlertDialog.Builder(this).setTitle("Select Kit").setItems(items, new DialogInterface.OnClickListener() { // from class: com.pramod.octapadpromidi.MainActivity.25                @Override // android.content.DialogInterface.OnClickListener
                public void onClick(DialogInterface dialog, int which) {
