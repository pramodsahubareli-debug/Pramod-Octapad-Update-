    ...
    private short[] decodePcmFromWav(byte[] wavData) {
        ...
        try {
            ...
            while (offset + 8 <= wavData.length) {
                String chunkId = new String(wavData, offset, 4, "ASCII");
                ...
                if ("fmt ".equals(chunkId)) {
                    ...
                    audioFormat = (wavData[chunkDataStart] & 0xFF) |
                            ((wavData[chunkDataStart + 1] & 0xFF) << 8);
                    numChannels = (wavData[chunkDataStart + 2] & 0xFF) |
                            ((wavData[chunkDataStart + 3] & 0xFF) << 8);
                    bitsPerSample = (wavData[chunkDataStart + 14] & 0xFF) |
                            ((wavData[chunkDataStart + 15] & 0xFF) << 8);
            ...
            if (audioFormat != 1 || numChannels <= 0 || dataOffset < 0 || dataSize <= 0 || (bitsPerSample != 8 && bitsPerSample != 16)) {
                return null;
            }

            int frameCount = dataSize / (numChannels * (bitsPerSample / 8));
            if (frameCount <= 0) {
                return null;
            }
            ...
            for (int frame = 0; frame < frameCount; frame++) {
                if (readPos + sampleSize > wavData.length) {
                    return null;
                }
                short sample;
                if (bitsPerSample == 16) {
                    sample = (short) ((wavData[readPos] & 0xFF) | (wavData[readPos + 1] << 8));
                } else {
                    int unsigned = wavData[readPos] & 0xFF;
                    sample = (short) ((unsigned - 128) << 8);
                }
                pcm[outIndex++] = sample;
                readPos += sampleSize * numChannels;
            }

            if (outIndex != pcm.length) {
                return null;
            }
            return pcm;
        } catch (Exception e) {
